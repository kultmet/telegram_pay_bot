...
from app.api import create_blacklist_object, create_payments, create_user, exists_user, get_blacklist_sync, get_payments, get_users, update_ballance

from app.buttons import fill_balance_buttons, admin_buttons, balance_buttons, user_management_buttons
from config import ADMINS, PAYMENTS_TOKEN, bot, dp
from constants import ERROR_LOGS, GREETING, INFO_LOGS, USER_BALLANCE, PAYMENT_MESSAGE
...
async def start(message: types.Message):
    ...
    try:
        ...
        bot_message = await message.answer(GREETING.format(username=message.from_user.username), reply_markup=fill_balance_buttons)
        message_stack.push(bot_message.message_id)
...
async def add_amount(call: types.CallbackQuery):
    ...
    try:
        bot_message = await call.message.answer('Введите сумму, на которую вы хотите пополнить баланс')
        message_stack.push(bot_message.message_id)
...
async def amount_reciver(message: types.Message, state: FSMContext):
    info_logger.info(f'amount_reciver: {message.from_user.id} send {message.text}')
    try:
        int(message.text)
    except ValueError:
        bot_message = await message.answer('Проверьте корректность ввода! Можно вводить тоько число!')
        message_stack.push(bot_message.message_id)
        message_stack.push(message.message_id)
        warning_logger.warning(f'uncorrect input: {message.from_user.id} send "{message.text}"')
        return
    bot_message = await message.answer(message.text)
    message_stack.push(bot_message.message_id)
    message_stack.push(message.message_id)
    try:
        price = types.LabeledPrice(label=f'Пополнить баланс на {message.text}', amount=int(message.text) * 100)
...
async def pre_cheskout_query(pre_checkout_q: types.PreCheckoutQuery):
    bot_message: types.Message = await bot.answer_pre_checkout_query(pre_checkout_q.id, ok=True)
...
async def successful_payment(message: types.Message):
    ...
    info_logger.info(f'successful_payment: complete')
    await delete_messages(message)
...
async def admin(message: types.Message):
    ...
    if message.from_user.id not in ADMINS:
        bot_message: types.Message = await message.answer('Админ панель не доступна')
        info_logger.info(f'admin: rejected')
        message_stack.push(bot_message.message_id)
    else:
        bot_message: types.Message = await message.answer('Добро пожаловать!', reply_markup=admin_buttons)
        await delete_messages(message)
        message_stack.push(bot_message.message_id)
        info_logger.info(f'admin: accepted')
...
async def users(call: types.CallbackQuery, buttons):
    try:
        ...
        for user in users:
            ...
            bot_message = await call.message.answer(USER_BALLANCE.format(**user, ballance=total_ballance/100), reply_markup=buttons)
            message_stack.push(bot_message.message_id)
            
    except TypeError:
        await call.answer('Что-то пошло не так, скорее всего проблеммы на сервере.')
...
async def payments(call: types.CallbackQuery):
    ...
    if payments:
        if not payments:
            bot_message = await call.message.answer(f'У {user_id} пользователя Платежей не было')
            message_stack.push(bot_message.message_id)
            info_logger.info(f'payments: None')
        for payment in payments:
            bot_message = await call.message.answer(PAYMENT_MESSAGE.format(**payment))
            message_stack.push(bot_message.message_id)
...
async def ballance_reciver(message: types.Message, state: FSMContext):
    info_logger.info(f'ballance_reciver: {message.from_user.id} send {message.text}')
    async with state.proxy() as data:
        user_id = data['user_id']
    await message.answer(message.text)
    try:
        await update_ballance(user_id, {'total_ballance': float(message.text)* 100})
